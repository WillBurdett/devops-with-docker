...
`solution`
(I'll submit the commands used instead as the answer on it's own is meaningless for learning)

```console
$ docker run -d --rm -it --name ubuntu_alpine devopsdockeruh/simple-web-service:alpine
$ docker exec -it ubuntu_alpine sh
$ vi text.log
```
...
`solution`
Just google the image name, go on docker hub and see the password in the read me. ('basics')
...
Add a final layer to the 'devopsdockeruh/simple-web-service:alpine' image that runs it on a server.
